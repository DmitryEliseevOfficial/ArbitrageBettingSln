...
namespace xNet
{
    ...
    public abstract class ProxyClient : IEquatable<ProxyClient>
    {
        ...
        public static ProxyClient Parse(string proxyData)
        {
            ProxyType type = ProxyType.Http;
            var temp = proxyData.ToUpper();
            if(temp.StartsWith("SOCKS5://"))
            {
                type = ProxyType.Socks5;
                proxyData = proxyData.Remove(0,9);
            }
            else if (temp.StartsWith("SOCKS4://"))
            {
                type = ProxyType.Socks4;
                proxyData = proxyData.Remove(0, 9);
            }
            else if (temp.StartsWith("SOCKS4a://"))
            {
                type = ProxyType.Socks4a;
                proxyData = proxyData.Remove(0, 10);
            }
            else if (temp.StartsWith("HTTP://"))
            {
                type = ProxyType.Http;
                proxyData = proxyData.Remove(0, 7);
            }
            else if (temp.StartsWith("HTTPS://"))
            {
                type = ProxyType.Http;
                proxyData = proxyData.Remove(0, 8);
            }

            return Parse(type, proxyData);
        }
